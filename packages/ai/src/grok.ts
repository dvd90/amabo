/**
 * grok.ts — a second voice for the Amarium (multi-LLM). xAI's Grok speaks an
 * OpenAI-compatible chat-completions dialect; this adapter translates the app's
 * Anthropic-shaped port (AnthropicLike: system blocks, a forced tool, tool_use
 * content) to and from it, so narrate()/narrateSymposium() and the whole metering
 * stack (LAUNCH_PLAN.md L3) work unchanged whichever provider is behind the port.
 *
 * Plain fetch, no SDK — same spirit as the API's monitor.ts and stripe.ts. Anthropic
 * model ids arriving through the port are MAPPED to the configured Grok tiers
 * (peek/milestone), never sent through. Any HTTP failure throws; narrate() already
 * catches and falls back to the local voice, so the creature never goes mute.
 */
...
export function makeGrokClient(cfg: GrokConfig, fetchFn: typeof fetch = fetch): AnthropicLike {
  const endpoint = `${cfg.baseUrl ?? 'https://api.x.ai/v1'}/chat/completions`;
  ...
  return {
    messages: {
      async create(rawBody: unknown) {
        ...
        if (!res.ok) throw new Error(`grok chat/completions → ${res.status}`);
