...
export function Holdings({ owner, disabled }: { owner?: Address; disabled: boolean }) {
  ...
  const owned = ids.filter(
    (_, i) => (owners?.[i]?.result as Address | undefined)?.toLowerCase() === owner?.toLowerCase(),
  );
  ...
  const { data: amounts, refetch } = useReadContracts({
    ...
    query: {
      enabled: tbaList.length === owned.length && owned.length > 0 && REWARD_TOKENS.length > 0,
    },
  });
  const cell = (i: number, t: number, k: 0 | 1) =>
    amounts?.[(i * REWARD_TOKENS.length + t) * 2 + k]?.result as bigint | undefined;
  ...
  return (
    <section>
      ...
      {REWARD_TOKENS.length === 0 && (
        <p className="muted">
          Set <code>NEXT_PUBLIC_REWARD_TOKENS</code> to show reward balances (VERIFY addresses
          first).
        </p>
      )}
      <table>
            ...
            <tr key={id.toString()}>
              ...
              <td>
                {REWARD_TOKENS.map((token, t) => {
                  ...
                  const fmt = (v?: bigint) =>
                    v !== undefined && decimals !== undefined ? formatUnits(v, decimals) : '…';
